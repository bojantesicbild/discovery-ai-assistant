    ...
    <header className="topbar">
      ...
      <div className="topbar-actions">
        <div ref={notifRef} style={{ position: "relative" }}>
          <button className="icon-btn notif-trigger" title="Notifications" onClick={() => setNotifOpen((o) => !o)}>
          ...
          {notifOpen && (
            <div className="notif-popover" role="dialog">
              {/* Header — title + counter so the popover reads as a proper
               *  drawer, not an anonymous list. */}
              <header className="notif-popover-head">
                <span className="notif-popover-title">Inbox</span>
                {(notifCount + activeReminderCount) > 0 && (
                  <span className="notif-popover-count">
                    {notifCount + activeReminderCount} open
                  </span>
                )}
              </header>

              {/* Tab pair — matches the .dp-subtab pattern (ink underline
               *  + accent count pill). */}
                ...
                {([
                  ...
                  return (
                    ...
                    >
                      {t.label}
                      {t.badge > 0 && (
                        <span className="notif-tab-badge">{t.badge > 99 ? "99+" : t.badge}</span>
                      )}
                    </button>
                  );
                })}
                      ...
                      <>
                        {notifTotal > 0 && (
                          <div className="notif-meta-row">
                            <span className="notif-meta-count">
                              <strong>{notifications.length}</strong> of <strong>{notifTotal}</strong>
                            </span>
                          </div>
                        )}
